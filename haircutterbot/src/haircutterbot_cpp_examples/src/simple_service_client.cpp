...
public:
    ...
    {
        client_ = create_client<haircutterbot_msgs::srv::AddTwoInts>("add_two_ints");

        auto request = std::make_shared<haircutterbot_msgs::srv::AddTwoInts::Request>();
        request->a = a;
        request->b = b;
...
private:
    rclcpp::Client<haircutterbot_msgs::srv::AddTwoInts>::SharedPtr client_;

    void responseCallback(rclcpp::Client<haircutterbot_msgs::srv::AddTwoInts>::SharedFuture future)
    {
        if(future.valid())
        {
